...
void setGdtDescriptor(int n, unsigned short limit, long base,
...
{
    setGdtDescriptor(task->pid+6, sizeof(task->tss) - 1, (long)&task->tss, 0x89, 0x00);
}
...
void GdtInstall()
{
    ...
    setGdtDescriptor(6, sizeof(tss) - 1, (long)&tss, 0x89, 0x00);

    gdtr.limit = sizeof(gdt_table) - 1;
    gdtr.base = (long)gdt_table;

    unsigned short selector = 6 * 8;
